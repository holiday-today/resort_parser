...
def my_async(hh, func):
    ...
    p4 = Process(target=func, args=([hh[(3*a):(4*a)], soups]))
    p5 = Process(target=func, args=([hh[(4*a):(5*a)], soups]))
    p6 = Process(target=func, args=([hh[(5*a):(6*a)], soups]))
    p7 = Process(target=func, args=([hh[(6*a):(7*a)], soups]))
    p8 = Process(target=func, args=([hh[(7*a):], soups]))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
